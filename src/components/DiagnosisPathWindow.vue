...
onMounted(async () => {
  unlistenPayload = await appWindow.listen<DiagnosisPathPayload>('diagnosis-path:update', ({ payload: nextPayload }) => {
    isLoading.value = true;
    statusPhase.value = 'rendering';
  ...
  unlistenStatus = await appWindow.listen<DiagnosisPathWindowStateEvent>(
    'diagnosis-path:status',
    ({ payload: nextState }) => {
      isLoading.value = Boolean(nextState?.loading);
      statusPhase.value = nextState?.phase || (isLoading.value ? 'preparing' : 'success');
      statusMessage.value = nextState?.message || (isLoading.value ? '正在生成诊断路径...' : '');
