...
18. 症状问诊和语音问诊最终一键回写共用 `record-confirmed` 构造器；进入 `diagList.idDiag` 的值必须是标准诊断库 ID（PHIS `ID_DIE`），不得使用 `diag_*`、`phis-diagnosis-*` 等前端临时 ID。`record-confirmed.outpatientRecord` 由 `features/clinical-result` 的完整门诊病历 builder 先统一生成和质控，再由 `writebackScope` 过滤为本次选择的主诉、现病史、既往史、个人史、家族史、体格检查和注意事项字段；未选字段、`diagList` 或 `orderList` 必须从 payload 中省略，HIS 保持原值。该对象不包含 `diagnosisText`，只有选择诊断时才由 HIS 根据 `diagList` 自动生成病历诊断行，避免诊断文书来源与标准诊断回写来源分叉。症状问诊从 `Diagnosis` 适配到共享结果页的 `VoiceIntentResult.diagnoses` 时必须把标准诊断 ID 透传为 `matchedItem.id` 或等价标准 ID 字段，避免共享结果页初始化时丢失诊断主键。
...
| `useClinicalResultWritebackScope.ts` / `ClinicalResultWritebackScopeSelector.vue` | 共享结果页部分回写能力：controller 按当前字段正文和已选诊断/治疗计算可用范围，门诊病历父级支持 checked / mixed / unchecked，字段级选择使用稳定 key；浮层组件只渲染锚定入口、数量、三态复选、空态和“未选内容保持 HIS 原值”说明。新患者/就诊 reset，重新生成通过前后可用性快照决定新增内容是否默认选择；最终作用域交给 `recordConfirmedPayload.ts` 决定字段省略，不允许 UI 直接拼装空数组 | [src/features/consultation-result/model/useClinicalResultWritebackScope.ts](src/features/consultation-result/model/useClinicalResultWritebackScope.ts) / [src/features/consultation-result/ui/ClinicalResultWritebackScopeSelector.vue](src/features/consultation-result/ui/ClinicalResultWritebackScopeSelector.vue) |
| `useClinicalRecordFactConfirmation.ts` / `clinicalRecordAnnotation.ts` / `ClinicalRecordAnnotatedText.vue` / `ClinicalRecordFactPanel.vue` | 共享结果页“病历事实核查”能力：既往史、个人史、家族史的标准模板由 `historyRecordTemplates.ts` 提供，模板识别仅用于内部候选去重与安全规则，不在字段级显示来源提示；controller 管理 AI 候选生成、医生核查状态、重点项阻断和缓存恢复；候选可消费病例相关性、正式诊断和病历书写完整性要求。`clinicalRecordAnnotation.ts` 同时切分正式事实与待核查候选：候选文本已存在于正文时直接覆盖原句为待核查片段，不再追加重复文本；正文中不存在时才作为独立阅读层候选显示且不修改 `modelValue`。`ClinicalRecordAnnotatedText.vue` 保持自然行高、换行与左对齐，以 `AI` / `!`、虚线和文字样式标识候选并提供锚点浮窗，不显示模板来源标签，也不在每个片段后重复 AI 来源标签；共享结果页只在存在 pending 候选时，于整个病历正文前显示一次全局说明。折叠汇总只突出待核查状态。模型请求、病例字段实际写入、toast、缓存与 PHIS 回写由页面显式注入或编排，不得把独立阅读层 DOM 文本当成正式病历 | [src/features/consultation-result/model/useClinicalRecordFactConfirmation.ts](src/features/consultation-result/model/useClinicalRecordFactConfirmation.ts) / [src/features/clinical-result/historyRecordTemplates.ts](src/features/clinical-result/historyRecordTemplates.ts) / [src/features/clinical-result/clinicalRecordAnnotation.ts](src/features/clinical-result/clinicalRecordAnnotation.ts) / [src/features/consultation-result/ui](src/features/consultation-result/ui) |
| `DiagnosisDifferentialList.vue` | 共享结果页待鉴别方向只读列表，只展示匹配度、依据和需补充信息，不提供选中、设主诊断、治疗刷新或回写动作 | [src/features/consultation-result/ui/DiagnosisDifferentialList.vue](src/features/consultation-result/ui/DiagnosisDifferentialList.vue) |
...
| `VoiceRecordFieldEditor.vue` | 共享结果页病例字段受控编辑器：文档态委托 `ClinicalRecordAnnotatedText.vue` 渲染原文内事实标记、未记录空态与确认浮窗，表单态继续使用 textarea；不再展示或接收病例字段反馈按钮、反馈弹层、草稿和提交事件，也不为这些已移除控件预留右侧空间。编辑器只转发受控字段值和事实确认事件，不自行生成事实、不写缓存或 PHIS；父页负责字段值、确认写入、用户日志和 PHIS 回写 | [src/features/voice-consultation/ui/VoiceRecordFieldEditor.vue](src/features/voice-consultation/ui/VoiceRecordFieldEditor.vue) |
