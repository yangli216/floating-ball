...
| `services/his/PhisHisAdapter.ts` | 默认厂商实现：thin wrapper，把 `HisService` 类（PHIS 形态）暴露为 `HisAdapter` 接口；详情、检查部位、患者信息与住院上下文方法在此处把 PHIS 字段映射为中性 DTO，诊疗项目详情会把 `idDeptExec` 映射为 `executingDeptId`、`count/amount/quantity` 映射为 `defaultQuantity`，其中体温单会从 `detail` 半结构化文本提取血压、呼吸、血氧等生命体征，目录与字典方法仍直接透传 | [src/services/his/PhisHisAdapter.ts](src/services/his/PhisHisAdapter.ts) |
