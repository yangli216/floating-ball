...
3.1 多慢病候选不直接进入同一份病历。`chronicRefillAssessment.ts` 为候选保存稳定的“慢病分组 + 历史临床诊断”选项，`ReceptionCapsule.vue` 先收集医生多选范围，再由纯规则构建仅包含已选诊断、相关就诊和可安全归属处方的 scoped candidate。病历模型、结果页核查、确定性 Builder、初始诊断和用药定稿均只消费该 scoped candidate。某次历史就诊同时含多个慢病、但 HIS 未提供处方与诊断的直接关联时，只有医生选中该就诊的全部慢病类型才允许自动继承其处方；部分选中时保留就诊文本证据，但不把无法归属的药品混入本次续方。
...
| `useClinicalRecordFactConfirmation.ts` / `clinicalRecordAnnotation.ts` / `ClinicalRecordAnnotatedText.vue` | 共享结果页“病历 AI 阅读提示”能力：既往史、个人史、家族史的标准模板由 `historyRecordTemplates.ts` 提供，模板识别仅用于内部候选去重与安全规则，不在字段级显示来源提示；controller 管理 AI 候选生成、当前会话移除和缓存恢复，候选可消费病例相关性、正式诊断和病历书写完整性要求。移除动作在内部保留 `dismissed` 墓碑以阻止缓存恢复后重新生成，但 UI、数量和字段查询只消费 `pending`，该状态不等同于拒绝建议或确认结果。`clinicalRecordAnnotation.ts` 同时切分正式事实与 AI 候选，并为候选片段保留正式字段中的精确范围：候选文本已存在于正文时直接覆盖原句为提示片段，不再追加重复文本；正文中不存在时只作为独立阅读层显示且不修改 `modelValue`；医生显式调整或移除时由该纯规则按片段范围替换/删除，未命中的阅读层候选只能在“应用”后追加进正式字段。`ClinicalRecordAnnotatedText.vue` 保持自然行高、换行与左对齐，所有候选统一显示 `AI` 标记，一般项使用蓝色文字与虚线，重点项使用红色文字与虚线；点击候选标记只打开锚定原文的非模态 AI 核查层，以候选表述、核查问题、必要依据、复制、调整和移除组成紧凑内容，不重复渲染重点/回写标签及说明；调整后的文本只有在医生点击应用后才进入 `modelValue`，调整或移除后由 controller 隐藏该候选，两者均不产生确认状态或回写门禁。非编辑态正文选区的快速复制也由该组件在本地管理，不新增全局状态。共享结果页只在存在 pending 候选时，于整个病历正文前显示一次全局说明，不再渲染底部核查面板，也不保留逐项确认状态或重点项回写门禁。模型请求、toast、缓存与 PHIS 回写由页面显式注入或编排，不得把未应用的独立阅读层 DOM 文本当成正式病历 | [src/features/consultation-result/model/useClinicalRecordFactConfirmation.ts](src/features/consultation-result/model/useClinicalRecordFactConfirmation.ts) / [src/features/clinical-result/historyRecordTemplates.ts](src/features/clinical-result/historyRecordTemplates.ts) / [src/features/clinical-result/clinicalRecordAnnotation.ts](src/features/clinical-result/clinicalRecordAnnotation.ts) / [src/features/consultation-result/ui](src/features/consultation-result/ui) |
| `useDifferentialDiagnosisDirection.ts` / `diagnosisSuggestionPresentation.ts` / `DiagnosisDifferentialList.vue` | 共享结果页待鉴别方向与医生转入流程：纯展示规则继续把低置信、证据不足和模型显式 differential 项隔离在正式诊断之外，并保证 AI 正式建议最多 3 项；局部 controller 只保存“已纳入诊疗方向 / 已由医生转为正式诊断”的稳定 key，支持 reset、缓存序列化和恢复，不创建 Pinia store。列表组件展示匹配度、依据、缺失信息和“纳入诊疗方向 / 补充依据 / 转为正式诊断 / 取消纳入”动作；纳入状态不写诊断、不触发治疗，补充依据复用 `ClinicalResultSupplementDialog.vue`，转入必须匹配标准诊断库。医生转入项不受 AI 三项截断，页面通过现有诊断选择 controller 把它加入已选次诊断并保留原主诊断；只有医生后续显式设为主诊并刷新方案才改变治疗依据。语音编辑快照同时保存方向状态、主诊断 identity 和全部已选正式诊断 identities，恢复时先恢复转入 key 再重建选择集合，避免转入次诊断在同就诊缓存恢复后丢失 | [src/features/consultation-result/model/useDifferentialDiagnosisDirection.ts](src/features/consultation-result/model/useDifferentialDiagnosisDirection.ts) / [src/features/clinical-result/diagnosisSuggestionPresentation.ts](src/features/clinical-result/diagnosisSuggestionPresentation.ts) / [src/features/consultation-result/ui/DiagnosisDifferentialList.vue](src/features/consultation-result/ui/DiagnosisDifferentialList.vue) |
...
| `auxiliaryRecommendationPresentation.ts` | 检查 / 检验推荐展示纯规则：把 `goal / goalGroup / goalGroupPurpose / necessity` 归一为直接可读的开立目的、优先级和临床目标分组；缺少新字段的历史缓存使用稳定兜底，不改变推荐项选中状态，不参与 PHIS / LIS 申请单分组。`TreatmentRecommendationSection.vue` 只消费其分组结果并渲染组标题、组目的与卡片列表 | [src/features/consultation-result/model/auxiliaryRecommendationPresentation.ts](src/features/consultation-result/model/auxiliaryRecommendationPresentation.ts) |
>
...
| `ReceptionCapsule.vue` | 接待胶囊（患者摘要、风险和门诊机会操作）；真实实现位于 reception 功能域，风险规则和风险详情组件仍由 reception-risk 提供，App 通过 `@features/reception` 公开入口消费。患者年龄 prop 使用完整 `ageText`，原样展示“岁 / 个月 / 天”及复合月日龄，并将同一文本交给头像解析 | [src/features/reception/ui/ReceptionCapsule.vue](src/features/reception/ui/ReceptionCapsule.vue) |
