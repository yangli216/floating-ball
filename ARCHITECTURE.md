...
4. 灵活模式下的推荐诊断 / 诊断鉴别 / 推荐用药 / 推荐检查，必须继续复用 `ConsultationPage.vue` 现有的诊断生成、标准库匹配、诊断路径与方案联动逻辑，不允许维护第二套轻量推荐口径。
5. 各模块的“确认”和“引用”语义必须拆分：主诉/现病史回写可以直接更新医生站草稿；诊断鉴别确认只记日志，不修改病历；推荐诊断、推荐用药、推荐检查的“引用”才真正进入 PHIS 保存闭环。
6. 灵活模式必须实现前置门禁：`diagnosis` 入口要求已有主诉和现病史；`differential` / `medication` / `examination` 入口要求已有主诉、现病史和当前诊断；若条件不足，页面需要给出明确提示并停留在可继续补全信息的位置。
...
12. `ConsultationPage.vue` 里的推荐诊断必须保持单选，并以当前选中诊断作为引用对象；推荐用药、检查检验则保留多选，并在分组级提供一次引入所选项的入口。对暂不支持 PHIS 引用的推荐项，应作为只读处置建议单独展示，避免被误当作检查项提交。
...
| `SettingsPanel.vue` | 系统设置（含文本/音频模型配置） | [src/components/SettingsPanel.vue](src/components/SettingsPanel.vue) |
...
| `audioRecorder.ts` | Web Audio API 录音 | [src/services/audioRecorder.ts](src/services/audioRecorder.ts) |
