...
11.1 门诊病历模板分析由独立的 `features/outpatient-emr` 承载，正式入口为 `POST /api/outpatient/emr/analyze`，不进入 `ConsultationPage.vue`，也不依赖住院模板缓存、住院上下文或住院提示词。HIS 每次请求显式传入 `visitId + templateId + templateName + templateHtml + templateDefinition + targetFieldIds + recordContext + requestId`；SDK 与 Bridge 严格校验原值，不生成 `requestId`，不修剪、不去重。`targetFieldIds` 是模板自带渲染器执行性别、年龄及其他联动规则后形成的“当前实际显示且需要分析的可写字段”权威白名单；PCIE 不执行 `relationscript`，不根据患者或模板元数据扩充范围，缺失、空、重复、带首尾空白、未知或只读目标都在模型调用前拒绝。`templateHtml` 与 `templateDefinition` 是同一套模板不可分割的两个必填部分：HTML 是正式渲染器输出的当前实例，负责真实布局、显示状态和当前显示值；JSON 是顶层章节定义数组，负责章节、字段、字典、映射扩展和关系元数据。纯定义解码器只读取 `ID / NAME / ARTICLE / eles` 及文档化的大写叶子属性；模板解析器在净化 HTML 后按稳定 ID 严格配对。每个非 `TYPE=import` 定义字段都必须在 HTML 恰好出现一次，HTML 不得多出非动作字段，字段 `ID / TYPE / NAME / READONLY / ARTICLE` 必须一致；章节按定义 `ID + ARTICLE` 与 HTML `data-id + data-article` 配对，字段所属章节由最近的章节祖先确定，渲染章节名与定义章节名允许不同并分别保留。定义动作元素可以不出现在 HTML；HTML 中出现的动作元素必须在定义中存在。模板对任一半缺失、重复 ID、身份不一致或 HTML 当前显示值与 JSON `TEXT` 不一致时以 `TEMPLATE_PAIR_MISMATCH` 拒绝，不提供单源预览或旧格式兜底。HTML 经 `shared/lib/safeHtml.ts` 净化形成真实安全预览，JSON 不再生成合成布局。模板占位 DOM 与 JSON `DEFAULTVALUE` 只用于展示；只有 JSON `BINDINGDATA[].VALUE/TEXT` 定义不可变字典项，`VALUE/TEXT` 归一化后同时为空的不可选占位被排除，其余带首尾空白、非法或重复项以 `INVALID_DICTIONARY_DEFINITION` 失败，字典目标缺少定义以 `MISSING_DICTIONARY_DEFINITION` 失败。字典当前 `VALUE/TEXT` 必须命中同一合法项且 `TEXT` 与 HTML 当前显示值一致，但只作为模板源元数据，不进入分析草稿或模型消息。定义中的 `RECORD_FIELD / AI_SUITABLE` 是扩展元数据权威来源；HTML 同时携带对应属性时必须一致，HTML 单边扩展不作为兼容输入。解析快照保留定义章节 ID、业务章节 ID、渲染章节名和定义章节名，并按显式映射、标准字段 ID、确定性字段别名、章节显式映射和确定性章节别名形成不可变字段映射；未知映射保持 `recordField=null`，同章节 `section-compose` 可多对一，直接字段或跨章节歧义以 `DUPLICATE_RECORD_FIELD_MAPPING` 失败。同一 `requestId` 只有在 `visitId + templateId + templateName + 原始 templateHtml + 原始 templateDefinition + targetFieldIds 集合 + patient + recordContext` 均一致时才幂等恢复。模板对 hash 固定为 `sha256("outpatient-emr-template-pair.v1:" + sha256(templateHtml UTF-8) + ":" + sha256(templateDefinition UTF-8))`。正式业务流固定为 `本地 Bridge/SDK → 计算模板对 hash → 签名 /v1/client/outpatient-emr/templates/snapshots/resolve 查询历史解析 → 命中则严格恢复完整字段快照；未命中则执行模板定义解码 + HTML 净化与严格配对、权威目标范围/字典/映射，并签名登记 /snapshots 新版本 → 一次签名 AI 调用 → 医生预览编辑 → 同源 builder → complete_consultation → record-confirmed → reference-feedback`。查询以当前机构、`templateId` 与 `templateHash` 精确命中；模板首次出现或两份原文任一字节变化都必须解析并新增 hash 版本，已有版本不得重复解析或登记。历史查询失败、命中快照 schema/字段损坏或新快照登记失败都在模型调用前终止，不允许本地重复解析、旧 schema 或其他实验路径兜底。快照只发送 `templateId/templateName/templateHash/templateHtml/templateDefinition/parseResult`，禁止携带就诊、患者、病历上下文、模型值或医生值。模型必须为每个目标字典字段显式返回 key，漏项为协议错误，显式空值或非法值保留为空待医生选择，绝不回退模板当前或默认选择。最终 `fieldValues + dictionarySelections + outpatientRecord + writebackScope` 均由同一个医生最终值 Map 构建，未映射字段仍通过 `fieldValues` 返回，章节组合按模板顺序确定性归并，不再调用模型；结果经既有事件总线进入 `record-confirmed`，HIS 按模板字段 ID 回填，PCIE 不负责 PHIS 的最终写入动作。桌面端不提供模板上传、粘贴、字段勾选或本地结果预览入口；后台“病历模板 → 门诊解析记录”只读展示渲染 HTML、结构定义 JSON 和合并解析快照。该能力在正式 HIS 接入前不承担早期实验协议兼容：旧 `htmlContent/templateSource/sourceFormat`、HTML 字典属性、JSON 单源预览和旧 schema 直接删除，未经用户明确要求不保留双读双写、别名或兜底。

11.2 `features/outpatient-emr/model/useVoiceOutpatientEmrWorkflow.ts` 负责语音问诊与动态门诊模板之间的跨页协调，`App.vue` 只连接入口、props 和 emits。`POST /api/consultation/start-voice` 可在患者对象中携带严格的 `outpatientEmr` 模板规格；Bridge、SDK 和前端协调层都只接受 `templateId/templateName/templateHtml/templateDefinition/targetFieldIds/requestId` 六个字段，并要求当前 `idVis` 明确存在。共享结果页仍由 `VoiceConsultationNew.vue` 完成回写范围选择、标准诊断/医嘱门禁和标准 `record-confirmed` 草稿构造；存在模板续接时只向协调层发出 `writeback-prepared`，不得先调用 `complete_consultation`。协调层从医生已选标准 payload 中构造不含 PHIS 私有 raw 的 `recordContext`，再复用 11.1 的快照、解析、AI 与预览链路。最终 `outpatientEmrWriteback.ts` 以模板 `fieldValues` 为模板回填权威值，同时保留共享结果页已选 `writebackScope/diagList/orderList`，模板映射只能更新已经被医生选中的标准病历字段，不能扩大 scope；最终只调用一次 `complete_consultation`、只产生一条同 `requestId` 的 `record-confirmed`，并等待一次 `reference-feedback`。独立 `/api/outpatient/emr/analyze` 不携带标准问诊 payload，继续输出诊断关闭和空医嘱的模板专用结果。

`recordContext` 的 SDK 边界只接受严格 JSON 对象；非有限数值、日期对象、`undefined`、函数、循环引用等非 JSON 输入必须显式拒绝，不得借序列化静默删键、转型或补值。模板字段/章节身份、名称和显式映射值也按原字符串校验，带首尾空白、空值或错误类型时拒绝，不先修剪再形成映射；结构化 JSON 的 `NAME` 必填且精确，HTML `data-name` 仅在完全省略时使用对应字段或章节 ID 作为显示名。HTML 只读和 AI 标记只接受精确 `true/false`，非法值不能降级为 `false`。模型根结果必须是对象；最终 builder 必须收到本次范围内逐字段完整的医生最终值和调用方时间戳，缺失时失败，不从模板或本机状态补齐。SDK 在业务事件分发前先验证门诊确认/取消 payload，Promise 与业务订阅共用同一完整性门禁。
...
| `outpatientEmr` | 门诊模板分析 | 当前模板安全解析、一次结构化 AI 分析、医生编辑和字段参数回传 |
...
- ✅ HIS 集成事件监听
  ...
  - `start-outpatient-emr-analysis` - 门诊病历模板分析；由独立 `useOutpatientEmrBridgeController` 订阅 `/api/outpatient/emr/analyze` 投递的当前模板与病历上下文，打开 `features/outpatient-emr` 工作视图；不进入通用 `useEventListeners` 的问诊状态机
...
1. `src-tauri/src/http_server.rs` 仅保留 HIS/SDK 所需的本地 `/api/consultation/*`、住院病历入口、门诊模板分析入口和事件回执通道；已删除 `/api/pmphai/*` 第三方代理。
