...
### RETRO-005: consultationId 与旧结果污染 [已解决，历史轮询契约已移除]

- **现象**: 历史轮询实现曾让 HIS 读到上一位患者的结果。
- **根因**: `consultationId` 未在每次新接诊时正确重置，或前端页面状态残留。
- **解决方案**: 严格在 `start-consultation` 事件处理中重置所有关联状态；当前唯一结果通道 `/api/consultation/events/ws` 的消费方继续校验 `event.id` 与 `consultationId`。
...
### RETRO-053: 正式界面追加解释设计意图的小字 [已解决]

- **现象**: 慢病复诊配药卡片已经有“待确认”状态和后续确认流程，卡片下方仍额外显示“医生确认后进入动态核实与处方定稿，不自动开药”，占用视觉空间并让正式产品界面呈现为设计说明稿。
- **根因**: 设计实现把内部流程边界和安全立场直接翻译成常驻辅助文案，没有先判断这段文字是否帮助医生完成当前操作。
- **解决方案**: 删除卡片下方的整行说明，保留任务名称、患者事实、必要状态和明确操作；安全边界继续由确认页、医生最终确认和回写门禁保证。
- **后续防护**: 已升级为 `AGENTS.md` Review 门禁 #14；新增辅助文字前必须证明用户需要依赖它作出决定或处理异常，不得用界面小字解释实现流程或设计意图。

### RETRO-054: 把慢病登记表主键当作当前就诊锚点导致草稿必然拒绝 [已解决]

- **现象**: 医生在两慢病 AI 推荐区勾选检查检验并点击“加入诊疗方案”后，界面固定提示“慢病医嘱草稿与当前患者不匹配”，无法进入诊疗方案工作台。
- **根因**: 草稿构造器使用慢病查询响应的 `idRecord` 作为 `patientAnchorId`，主应用校验却使用当前患者的 `idVis / visitId`；`idRecord` 是两慢病登记表主键，`idVis` 是本次门诊就诊主键，两者没有相等语义。
...
### RETRO-056: 最外层拖拽标记被慢病页内部元素架空 [已解决]

- **现象**: 两慢病接诊插件收起和展开后都无法从患者区域拖动主窗口，展开态顶部虽然呈现为标题区但按住鼠标没有反应。
- **根因**: `data-tauri-drag-region` 只放在 `ReceptionCapsule` 最外层，鼠标实际命中内部患者信息元素时不会自动继承；收起态患者胶囊还显式使用了 `-webkit-app-region: no-drag`。
- **解决方案**: 把收起态患者胶囊和展开态患者标题区定义为显式拖拽把手，经 typed emit 交由主壳调用既有 Tauri `startDragging()`；统一过滤嵌套关闭按钮及其他交互控件。
- **后续防护**: 自定义无原生标题栏页面不能用祖先节点上的拖拽标记推断子元素可拖动；必须覆盖“把手空白/文本可拖、嵌套按钮不拖”的回归测试，并保留 Tauri `allow-start-dragging` capability。

### RETRO-057: 慢病检查检验另开直存通道导致既有回写闭环回归 [已解决]

- **现象**: 两慢病 AI 推荐可以展示并进入诊疗方案工作台，但医生最终确认后检查检验无法按原智医助手链路回写；此前已经验证通过的 `record-confirmed + reference-feedback` 行为发生回归。
- **根因**: 为复用 `ClinicDoctorCoreService.loadVisCliList` 的调入映射能力，同时新增了慢病独有 `saveOdsImp` 最终保存分支，把“映射门禁”和“保存协议”错误地绑定在一起；该分支绕过共享回执状态机，还允许用目录 `idSrv` 兜底缺失的 `idCli`，使页面成功语义、HIS 保存结果和标准诊断来源出现分叉。
- **解决方案**: 删除慢病直存分支，最终诊断和检查检验统一由共享 `record-confirmed` 发送并等待匹配的 `reference-feedback`；`loadVisCliList` 只作为目录项目的可选 PHIS 字段增强，不再承担最终保存。慢病诊断改为按病种标签匹配 HIS 标准诊断：高血压限定 `I10`，2 型糖尿病限定 `E11`，双病种同时携带。
- **后续防护**: 对接新的 HIS 查询或映射 RPC 时必须分别评估“读能力”和“写协议”，不得因复用读取结果自动引入第二条保存通道；任何最终回写分支都必须覆盖 requestId/consultationId 关联、快速回执竞态、失败保留现场和唯一 transport 回归测试。

### RETRO-058: 可选调入映射被误设为慢病推荐展示硬门禁 [已解决]

- **现象**: LLM 已基于当前 HIS 返回的可开立检查/检验目录生成推荐，但页面显示“暂无可直接回写的推荐”，所有候选都被标记为“当前 HIS 未返回调入映射”并过滤。
- **根因**: `queryAvailableExamLabItems` 已经按当前用户、机构与科室返回真实可开立目录，客户端又把 `loadVisCliList` 的 `idCli` 响应设为第二次展示准入；当该 RPC 未发布、返回空列表或只返回部分增强字段时，真实目录候选被错误等同为不可用。
