...
### RETRO-112: 把模板字典定义当成普通默认文案并丢失编码 [已解决]

- **现象**: 真实门诊模板中的“否认、无病容、柔软”等字典内容在入出演示中被当成普通默认文本处理；同时回参只保留显示文本，丢失了模板 `BINDINGDATA` 中的 `VALUE` 编码。
- **根因**: 解析层只抽取了 `BINDINGDATA[].TEXT` 为字符串白名单，没有完整保留 `VALUE/TEXT` 对，也没有把 `.placeholder / DEFAULTVALUE` 的展示提示、字段当前 `VALUE/TEXT` 和普通自由文本基线分成不同语义。HTML 渲染结果只含当前显示值，又被错误地当成能够代表完整字典的模板定义。
- **解决方案**: 新增独立模板字典 resolver：结构化模板完整保留并校验每个 `{ value, text }`，模型返回编码或文本均归一到原项；最终 payload 以 `fieldValues` 返回显示文本、以 `dictionarySelections` 返回字典编码和文本。字典字段在本地预览中改用下拉选择，缺少定义、最终为空或模板外值均阻止确认；只有带完整字典的结构化模板或显式携带 `data-dictionary-items` 的 HTML 可以安全处理字典字段。
- **后续防护**: 动态病历模板测试必须覆盖编码/文本双向匹配、空值确认门禁、模板外值拒绝、VALUE/TEXT 冲突、重复字典标识、男性月经字段整组省略，以及仅有渲染文本但缺少完整选项的 HTML 失败场景；不得再从字段类型或当前显示文本猜测字典全集。

### RETRO-113: 把模板字典当前选择当成本次模型分析结果 [已解决]

- **现象**: 模型漏掉字典字段或返回“没有高血压吧”等模板外文本时，归一化层会静默回退模板当前 `VALUE + TEXT`，导致最终参数看似完整，却无法证明模型本次真正分析过该字段；医生也看不到哪些字段需要人工选择。
- **根因**: 实现把“模板源当前选择合法”与“本次模型已显式返回合法结果”合并成同一个有效性判断，并在分析草稿初始化和模型归一化两个阶段都复用了模板基线。
- **解决方案**: 保留模板当前选择只用于模板源解析和一致性校验；分析草稿对所有生效字典字段从空值开始，模型必须逐字段显式返回，编码或文字命中时归一为模板项，缺失、空值或无法匹配时保持空值。预览以“请选择”展示待选字段，医生只能从模板字典补选，全部补齐后才允许构造最终参数。
- **后续防护**: 测试必须同时覆盖“已有模板默认项但模型漏项”“已有模板默认项但模型返回非法值”“模型返回合法编码/文字”“医生选择与默认项相同”四类场景，并断言前两类不会继承模板当前值、分析本身可完成、最终参数在医生补选前保持禁用。
