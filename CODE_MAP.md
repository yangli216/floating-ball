...
| 修改独立诊疗方案推荐 | [TreatmentPlanPage.vue](src/features/treatment-plan/ui/TreatmentPlanPage.vue) + [TreatmentPlanGroup.vue](src/features/treatment-plan/ui/TreatmentPlanGroup.vue) + [useTreatmentPlanRecommendations.ts](src/features/treatment-plan/model/useTreatmentPlanRecommendations.ts) + [useTreatmentPlanWriteback.ts](src/features/treatment-plan/model/useTreatmentPlanWriteback.ts) + [odsImportPayload.ts](src/features/treatment-plan/model/odsImportPayload.ts) + [odsImportSubmission.ts](src/features/treatment-plan/model/odsImportSubmission.ts) + [features/clinical-result](src/features/clinical-result) + [features/consultation-result](src/features/consultation-result)；入口来自 `/api/consultation/assist` 的 `action: treatment_plan`，只做四类治疗方案聚合清单，不进入 `ConsultationPage.vue`；AI 请求、标准库匹配、推荐项二次编辑、药品/项目 hydrate、库存校验、完整门诊病历 `outpatientRecord` 和 `record-confirmed` 构造必须继续复用共享 clinical-result / consultation-result 能力；仅 `sourceModule=chronic_disease` 的检查/检验草稿先经 `loadVisCliList` 映射并在最终确认时改走 `saveOdsImp`，不得同时发送 `record-confirmed` |
...
| **ReceptionCapsule.vue** | ~700 | 患者接诊胶囊与慢病四步折叠区编排；第 3 步复诊配药展示拆到 `ReceptionChronicRefillSection.vue`，医生确认后继续进入既有动态确认与可编辑病历结果页；判定与生成逻辑仍位于 `src/features/reception-risk/lib|api` |
...
| 第 3 步慢病复诊配药展示 | `src/features/reception/ui/ReceptionChronicRefillSection.vue`；只负责候选/空态/生成中 UI 与确认事件，不复制 `features/reception-risk` 业务规则 |
...
| 慢病 AI 推荐到诊疗方案 seed | `src/features/chronic-disease/api/chronicAiRecommendationService.ts` 从既有 HIS 可开立检查/检验目录生成带 `matchedItem` 的 AI 推荐；`src/features/chronic-disease/model/useChronicAiRecommendations.ts` 管理医生选择；`src/features/chronic-disease/ui/ChronicAiRecommendationPanel.vue` 展示；`src/features/chronic-disease/lib/chronicTreatmentPlanDraft.ts` + `src/features/treatment-plan/model/treatmentPlanInitialDraft.ts` 传递患者绑定的标准项目快照 |
