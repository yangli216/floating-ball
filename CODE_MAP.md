...
| **recommendationPreferenceTracker.ts** | -- | 区域化推荐偏好采集与轻量重排入口：只学习目录匹配后的诊断/医嘱标准项，最终确认、手动匹配和 probable match 确认写入本地队列并通过 `/v1/client/recommendation-preferences/events/batch` 批量上传；诊断/医嘱完成 `medicalDataService` 匹配后可调用 `/v1/client/recommendation-preferences/rank` 获取服务端 boost，但只重排已有候选、不新增候选、不修改 Prompt |
