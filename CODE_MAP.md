...
| 修改检验检查报告解读 | [ReportInterpretationWindow.vue](src/features/report-interpretation/ui/ReportInterpretationWindow.vue) + [ReportInterpretationWorkspace.vue](src/features/report-interpretation/ui/ReportInterpretationWorkspace.vue) + [ReportSourcePreview.vue](src/features/report-interpretation/ui/ReportSourcePreview.vue) + [ReportInterpretationContent.vue](src/features/report-interpretation/ui/ReportInterpretationContent.vue) + [reportInterpretationPresentation.ts](src/features/report-interpretation/lib/reportInterpretationPresentation.ts) + [useReportInterpretationWorkspace.ts](src/features/report-interpretation/model/useReportInterpretationWorkspace.ts) + [reportedReportHistory.ts](src/features/report-interpretation/api/reportedReportHistory.ts) + [PhisHisAdapter.ts](src/services/his/PhisHisAdapter.ts) + [reportInterpretation.ts](src/services/reportInterpretation.ts) + [reportInterpretation.ts](src/types/reportInterpretation.ts) + [useEventListeners.ts](src/composables/useEventListeners.ts) + [useReceptionController.ts](src/app/events/useReceptionController.ts) + [http_server.rs](src-tauri/src/http_server.rs) + [sdk/med-hermes-sdk.js](sdk/med-hermes-sdk.js) + [report-interpretation-test.html](web_project/public/report-interpretation-test.html)；外部单报告请求保留独立窗口，接诊发现近 14 天已出报告时进入应用内报告工作台；工作台默认以统一报告单样式显示结构化原始报告，医生手动触发后才生成 AI 解读，并通过原始报告 / AI 解读双视图切换；定性阳性、异常标记、非正常方向和数值越界会交叉复核上游 normal 标记，异常表只使用确定性字段；左侧就诊科室来自 `HisVisitRecord.deptName` |
| 修改门诊接诊与风险胶囊 | [useReceptionController.ts](src/app/events/useReceptionController.ts) + [receptionFlowGuard.ts](src/app/events/receptionFlowGuard.ts) + [receptionPatientSummary.ts](src/features/reception/lib/receptionPatientSummary.ts) + [reportedApplyResults.ts](src/features/reception/lib/reportedApplyResults.ts) + [useReceptionSessionController.ts](src/features/reception/model/useReceptionSessionController.ts) + [useOutpatientScenarioRouter.ts](src/features/reception/model/useOutpatientScenarioRouter.ts) + [ReceptionCapsule.vue](src/features/reception/ui/ReceptionCapsule.vue) + [features/reception-risk](src/features/reception-risk)；患者补全、风险评估和复诊机会异步写入必须复用同一 flow token，风险评估失败只降级风险能力，不得把已成功的患者接诊标记为失败；两慢病入口在取得 `idCard` 后调用 `HisAdapter.fetchChronicDiseasePatientVisitHistory(idCard)`，原 `body` 字段直接并入 `PatientContext.raw`；患者展示信息从 `currentPatient` 派生，风险和机会状态只经 session controller action 修改；患者字段兼容读取、已出报告申请单判断和病史摘要属于纯 lib，具体门诊场景导航由统一路由执行 |
| 修改复诊配药 | [chronicRefillAssessment.ts](src/features/reception-risk/lib/chronicRefillAssessment.ts) + [twoChronicRefillEvidence.ts](src/features/chronic-disease/lib/twoChronicRefillEvidence.ts) + [chronicRefillConfirmation.ts](src/features/reception-risk/lib/chronicRefillConfirmation.ts) + [chronicRefillConfirmation.ts](src/features/reception-risk/api/chronicRefillConfirmation.ts) + [useChronicRefillConfirmation.ts](src/features/reception-risk/model/useChronicRefillConfirmation.ts) + [ChronicRefillConfirmationPage.vue](src/features/reception-risk/ui/ChronicRefillConfirmationPage.vue) + [chronicRefillRecord.ts](src/features/reception-risk/api/chronicRefillRecord.ts) + [chronicRefillInventory.ts](src/features/reception-risk/lib/chronicRefillInventory.ts) + [clinicalResultMedicineQuantity.ts](src/features/clinical-result/clinicalResultMedicineQuantity.ts) + [clinicalResultNarrative.ts](src/features/clinical-result/clinicalResultNarrative.ts) + [useReceptionController.ts](src/app/events/useReceptionController.ts) + [HisAdapter.ts](src/services/his/HisAdapter.ts) + [PhisHisAdapter.ts](src/services/his/PhisHisAdapter.ts) + [hisService.ts](src/services/hisService.ts) + [clinicalResultContract.ts](src/features/clinical-result/clinicalResultContract.ts) + [VoiceConsultationNew.vue](src/components/VoiceConsultationNew.vue)；历史查询必须携带当前 `idVis` 以排除本次就诊；PHIS 历史药品属性以 `presList[].presSubList[]` 为主并映射为中性 `HisHistoricalMedication`，`orderList` 只负责医嘱分类、检验检查关联和缺失兜底；两慢病直接打开时 `twoChronicRefillEvidence.ts` 从原始 `diagnosis / rqflStatus / visitInfos[].drugList[]` 生成 supplemental evidence，不改写 `HisPatientHistory`、不消费当前诊中处方；慢病候选只要求历史慢病诊断，历史药品为空时继续由模型结合诊断和当前库存推荐；确认计划由模型按上下文动态生成 3～5 项并给推荐值，文字/语音补充不回流确认项，只在最终病历调用中作为医生明确说明压缩入现病史；正文药品仅保留规范名称；复诊药品天数只沿用可可靠关联的最近历史处方，模型天数不进入权威处方；当前主诉/现病史/诊断出现报告回诊语义时必须抑制复诊配药候选；模型理由只承载临床依据，剂量与包装换算由共享能力完成；通过 `recommendationPolicy` 禁止结果页自动补拉药品、检查、检验或处置 |
...
| 修改客户端更新源 | [UpdateChecker.vue](src/features/settings/ui/UpdateChecker.vue) + [ForceUpdateGate.vue](src/features/settings/ui/ForceUpdateGate.vue) + [updateCheckerStatus.ts](src/features/settings/model/updateCheckerStatus.ts) + [updateConfig.ts](src/services/updateConfig.ts) + [lib.rs](src-tauri/src/lib.rs)；强更策略是门禁事实源，门禁生效时即使 updater 没有返回安装包也不得展示“当前已是最新版本”；内网发布端见 `../floating-ball-server/modules/release` |
...
| 修改医学数据匹配 / HIS 出站标准服务 | [medicalData.ts](src/services/medicalData.ts) + [his/HisAdapter.ts](src/services/his/HisAdapter.ts) + [his/types.ts](src/services/his/types.ts) + [his/PhisHisAdapter.ts](src/services/his/PhisHisAdapter.ts) + [his/MockHisAdapter.ts](src/services/his/MockHisAdapter.ts) + [his/registry.ts](src/services/his/registry.ts) + [hisService.ts](src/services/hisService.ts) + [availableMedicineInventory.ts](src/features/clinical-result/api/availableMedicineInventory.ts) + [medical_catalog.rs](src-tauri/src/commands/medical_catalog.rs)；重点核对 SDK handshake 传入的 `orgCode / idTet` 是否进入缓存上下文，诊疗项目是否按机构+租户隔离，药品是否按机构+租户+药房 `storeId` 隔离；有效库存目录必须经 `HisAdapter.fetchAvailableMedicineInventory()` 获取，PHIS 侧经 `api/phis.aiAdapterService/queryInvSubList` 调用院端适配入口并按 `idMedPro` 合并批次，AI 缓存按机构+租户+药房隔离，库存校验单价按 `storeId + idMedPro` 从目录合并项解析且不得回退药品详情；住院上下文是否使用 PHIS `idAdsn`（中性 `admissionId`）或 `patientId / inpatientVisitId / encounterId / inpatientNo` 锚定同患者多次住院，住院病历 AI 上下文是否只经 `fetchInpatientEmrContext` / PHIS `buildContext` 聚合入口返回，以及服务端暂不可用时既有缓存仍可恢复 |
...
| **diagnosisChecklist.ts** | ~70 | 诊断鉴别 checklist 纯规则：统一共享结果页与独立鉴别诊断窗口的响应类型、LLM JSON 解析、条目归一、关键诊断不匹配判断和风险项映射 | 不调用 LLM、不持有 Vue 状态、不弹 toast、不处理窗口生命周期 |
| **useClinicalResultDiagnosisChecklist.ts** | ~90 | 共享结果页诊断鉴别 controller：管理弹窗开合、当前诊断、请求生命周期、迟到响应失效、空态和错误态；Prompt/LLM 请求、渠道 trace、错误文案与 toast 由页面注入 | 不服务独立鉴别诊断窗口的折叠和 watch 生命周期，不调用 PHIS 回写 |
...
| **VoiceSessionFeedbackBar.vue** | -- | 语音问诊整页反馈浮层主体；真实实现已迁至 `src/features/voice-consultation/ui/VoiceSessionFeedbackBar.vue`，旧 `src/components/VoiceSessionFeedbackBar.vue` 兼容包装已删除 | 一键回写成功回执后弹出，收集评分、点评、整体问题标签；提交或暂不反馈后收起结果页 |
...
| **ReceptionCapsule.vue** | ~700 | 患者接诊胶囊与慢病四步折叠区编排；收起态胶囊和展开态患者标题区通过 typed emit 请求主壳执行窗口拖拽，嵌套操作控件继续独立响应；第 2 步复诊配药展示拆到 `ReceptionChronicRefillSection.vue`，医生确认后继续进入既有动态确认与可编辑病历结果页；判定与生成逻辑仍位于 `src/features/reception-risk/lib|api` |
...
| 服务 | 行数 | 职责 |
|------|------|------|
| **hisService.ts** | -- | @internal 底层 PHIS HTTP 客户端；仅供 `services/his/*` 包装使用。业务代码禁止跨层 import，必须走 [his](src/services/his/index.ts) 入口（业务调用 `getHisAdapter()`；SDK handshake 走 `getHisService()`）；底层 `post/get` 会写入 HIS 联调出站日志；诊断目录按 1000 条/页循环同步，避免弱网下一次性拉取数万条超时；封装检查项目部位 / 方式候选查询；两慢病查询和保存分别调用 Adapter 发布的 `queryPatientVisitHistoryData` / `saveTcdForm`，请求和响应保留慢病系统原字段 |
| **hisIntegrationLog.ts** | -- | HIS 联调日志 Tauri 客户端：结构化记录、查询、清空、导出 Bridge / PHIS 调用流水 |
| **his/HisAdapter.ts** | -- | HIS 适配器接口契约；通用 HIS 能力使用中性 DTO，两慢病 `fetchChronicDiseasePatientVisitHistory(idCard)` 明确保留真实系统原字段，不增加改名层 |
...
| 职责 | 位置 |
| --- | --- |
| 接诊三态外壳与四步编排 | `src/features/reception/ui/ReceptionCapsule.vue` |
| 第 2 步慢病复诊配药展示 | `src/features/reception/ui/ReceptionChronicRefillSection.vue`；只负责候选/空态/生成中 UI 与确认事件，不复制 `features/reception-risk` 业务规则 |
| 患者慢病摘要与真实趋势展示计算 | `src/features/chronic-disease/lib/chronicDiseaseSummary.ts`；直接读取 `PatientContext.raw` 中的 `pressureList / pressureHist|pressureHList / gluList / visitInfos / drugList` 原字段 |
| 两慢病复诊配药补充证据 | `src/features/chronic-disease/lib/twoChronicRefillEvidence.ts`；仅在两慢病直接打开场景读取原始 `diagnosis / rqflStatus / visitInfos[].drugList[]`，要求明确管理病种与历史随访，不生成中性病历或结构化处方属性 |
...
| 正式随访 HIS 事件桥 | `src/features/chronic-disease/api/chronicDiseaseApi.ts`；独立窗把原始 `TcdVisitForm` 定向交给主窗口 Adapter 保存，不经过区域后端 |
| 健康处方 AI 草稿 | `src/features/chronic-disease/api/healthPrescriptionService.ts`；复用签名 `/v1/ai/chat` 并提供受控降级 |
| 独立业务窗创建与 ready/ack | `src/features/chronic-disease/api/chronicDiseaseWindowService.ts` |
| 年度自然年聚合与 AI 评估 | `src/features/chronic-disease/lib/annualAssessment.ts` 只筛选真实年度事实；`src/features/chronic-disease/api/annualAssessmentService.ts` 生成六类结构化医生评估草稿并提供受控降级；`src/features/chronic-disease/ui/AnnualAssessmentEvidencePanel.vue` 与 `AnnualAssessmentResult.vue` 分别展示事实和 AI 结果 |
| 慢病 AI 推荐到诊疗方案 seed | `src/features/chronic-disease/api/chronicAiRecommendationService.ts` 把真实慢病数据与当前 HIS 可开立检查/检验目录交给 LLM，并只接受目录 `catalogRef`；`src/features/chronic-disease/model/useChronicAiRecommendations.ts` 直接展示目录确认项目、管理医生选择，并在进入工作台前尽力补充 `loadVisCliList` 映射；`src/features/chronic-disease/ui/ChronicAiRecommendationPanel.vue` 展示；`src/features/chronic-disease/lib/chronicTreatmentPlanDraft.ts` + `src/features/treatment-plan/model/treatmentPlanInitialDraft.ts` 传递患者绑定的标准项目及第 2 步复诊配药病历上下文快照，`src/features/treatment-plan/model/useTreatmentPlanRecommendations.ts` 优先消费该快照，避免再次只读接诊时 HIS 原始字段 |
| 趋势图与业务窗 UI | `src/features/chronic-disease/ui/`；融合随访章节组件位于 `ui/follow-up/`，只通过显式 `v-model` 接收原实例表单状态 |
| 两慢病 Mock HIS 唤起页 | `web_project/public/chronic-disease-mock.html`；通过正式 SDK `openChronicDisease()` 和 `/api/chronic-disease/open` 直接唤起真实两慢病展开详情，并用 `idCard / pressureList / pressureHList / gluList / visitInfos / drugList` 原字段 fixture 模拟真实接口结果；不发送 `risks`，不在网页内渲染插件 |
