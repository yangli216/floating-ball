...
| 修改住院病历辅助生成 | [InpatientEmrPage.vue](src/features/inpatient-emr/ui/InpatientEmrPage.vue) + [useInpatientEmrGeneration.ts](src/features/inpatient-emr/model/useInpatientEmrGeneration.ts) + [inpatientEmrService.ts](src/features/inpatient-emr/api/inpatientEmrService.ts) + [inpatientEmrTemplate.ts](src/features/inpatient-emr/lib/inpatientEmrTemplate.ts) + [inpatientEmrPrompts.ts](src/features/inpatient-emr/lib/inpatientEmrPrompts.ts) + [his/types.ts](src/services/his/types.ts) + [HisAdapter.ts](src/services/his/HisAdapter.ts) + [http_server.rs](src-tauri/src/http_server.rs) + [sdk/med-hermes-sdk.js](sdk/med-hermes-sdk.js) + [sdk_test.html](docs/sdk_test.html) + [api.md](./api.md) + [三方对接手册](docs/his-inpatient-emr-ai-context-integration.md)；入口来自 `/api/inpatient/emr/generate` 或 `sdk.generateInpatientEmr(...)`，必须传 `admissionId + templateId + templateName + htmlContent`，可选传 `recordTime` 指定本次病程记录书写时间，也可传 `doctorSupplement` 作为医生补充要点，并可传 `contextPolicy + hisContext` 让 HIS 直接提供裁剪后的 AI 上下文包；小球切换为独立病历生成界面，按“获取住院上下文 / 整理诊疗摘要 / 整理病历依据 / 解析病历 / AI 生成”步骤展示进度；生成上下文包含 `documentContext + aiContext`，加载优先级为请求 `hisContext` -> HIS Adapter 聚合接口 `fetchInpatientEmrContext`（PHIS Adapter 直连 `api/phis.aiInpatientEmrContextService/buildContext` 一次性获取），不再回退旧住院登记/医嘱/体温单分散接口，AI 生成以 `recordTime` 日期作为今日/本次查房日期，历史体温单只能按最近一次记录引用；历史病历上下文由 HIS 侧裁剪，病案首页过滤，入院记录提取主诉、现病史和结构化章节，病程记录保留近期摘要；区域化模式下模板解析走后端 `/v1/client/inpatient-emr/templates/resolve` 并按 `templateId` 缓存，接收后台维护的字段提示词，非区域化本地解析兜底；左侧只展示 AI 生成字段，字段提示词点击详情后展开；医生在病历预览中直接调整高亮 AI 字段，非 AI 字段只读；重新生成会先弹出补充要点面板，支持文本录入、常用片段和语音转写，补充内容进入下一次 AI 生成；确认后通过 `record-confirmed` 结果通道把适合 AI 生成的 `{ [data-id]: 文本 }` `fieldValues` 回写给 HIS，不返回 `htmlContent`；收到 `reference-feedback` 成功回执后收起回小球 |
