...
| **ReceptionCapsule.vue / ChronicRefillReviewPanel.vue** | ~500 / ~200 | 接诊胶囊展示慢病“复诊配药”候选并仅在多慢病时本地选择本次范围；复诊核查面板以主诊断卡片按钮为锚点，用紧凑扁平列表展示同次生成返回的可选参考项，支持关闭和重开，未处理不阻断回写，未选慢病及无法按病种归属的混合处方不自动混入 |
