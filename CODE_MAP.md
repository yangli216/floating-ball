...
| 修改复诊配药 | [chronicRefillAssessment.ts](src/features/reception-risk/lib/chronicRefillAssessment.ts) + [chronicRefillRecord.ts](src/features/reception-risk/api/chronicRefillRecord.ts) + [chronicRefillInventory.ts](src/features/reception-risk/lib/chronicRefillInventory.ts) + [clinicalResultMedicineQuantity.ts](src/features/clinical-result/clinicalResultMedicineQuantity.ts) + [clinicalResultNarrative.ts](src/features/clinical-result/clinicalResultNarrative.ts) + [useReceptionController.ts](src/app/events/useReceptionController.ts) + [HisAdapter.ts](src/services/his/HisAdapter.ts) + [PhisHisAdapter.ts](src/services/his/PhisHisAdapter.ts) + [hisService.ts](src/services/hisService.ts) + [clinicalResultContract.ts](src/features/clinical-result/clinicalResultContract.ts) + [VoiceConsultationNew.vue](src/components/VoiceConsultationNew.vue)；历史查询必须携带当前 `idVis` 以排除本次就诊；PHIS 历史 `orderList` 仅 `sdOrd/sdSrv=11` 或存在药品主键的记录进入历史用药，检验检查不得混入；慢病候选只要求历史慢病诊断，历史药品为空时继续由模型结合诊断和有效库存推荐；当前主诉/现病史/诊断出现报告回诊语义时必须抑制复诊配药候选，胶囊不得展示有限历史窗口内的具体就诊次数；慢病分类组只负责候选识别，临床诊断必须保留历史原始具体名称并用于初始标准库匹配；诊断依据与用药依据分离，历史明确诊断按 `explicit/high` 展示且共享文案去重；草稿主诉/现病史必须表达具体慢病续方语义；模型理由只承载临床依据，剂量与包装换算由 `clinicalResultMedicineQuantity.ts` 根据最终处方字段统一计算；药品按“库存同品 → 库存等效药 → 规范通用名兜底”处理，无库存标准名保持未选中；通过 `recommendationPolicy` 禁止结果页自动补拉药品、检查、检验或处置 |
