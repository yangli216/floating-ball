...
| 修改门诊病历模板分析 | [OutpatientEmrPage.vue](src/features/outpatient-emr/ui/OutpatientEmrPage.vue) + [OutpatientEmrAnalysisPanel.vue](src/features/outpatient-emr/ui/OutpatientEmrAnalysisPanel.vue) + [OutpatientEmrTemplatePreview.vue](src/features/outpatient-emr/ui/OutpatientEmrTemplatePreview.vue) + [OutpatientEmrCombinedWritebackSummary.vue](src/features/outpatient-emr/ui/OutpatientEmrCombinedWritebackSummary.vue) + [useOutpatientEmrAnalysis.ts](src/features/outpatient-emr/model/useOutpatientEmrAnalysis.ts) + [useVoiceOutpatientEmrWorkflow.ts](src/features/outpatient-emr/model/useVoiceOutpatientEmrWorkflow.ts) + [voiceOutpatientEmr.ts](src/features/outpatient-emr/lib/voiceOutpatientEmr.ts) + [outpatientEmrService.ts](src/features/outpatient-emr/api/outpatientEmrService.ts) + [outpatientEmrTemplateSnapshotService.ts](src/features/outpatient-emr/api/outpatientEmrTemplateSnapshotService.ts) + [outpatientEmrTemplateDefinition.ts](src/features/outpatient-emr/lib/outpatientEmrTemplateDefinition.ts) + [outpatientEmrDictionary.ts](src/features/outpatient-emr/lib/outpatientEmrDictionary.ts) + [outpatientEmrFieldMapping.ts](src/features/outpatient-emr/lib/outpatientEmrFieldMapping.ts) + [outpatientEmrRequestIdentity.ts](src/features/outpatient-emr/lib/outpatientEmrRequestIdentity.ts) + [outpatientEmrTemplate.ts](src/features/outpatient-emr/lib/outpatientEmrTemplate.ts) + [outpatientEmrWriteback.ts](src/features/outpatient-emr/lib/outpatientEmrWriteback.ts) + [useOutpatientEmrBridgeController.ts](src/app/events/useOutpatientEmrBridgeController.ts) + [VoiceConsultationNew.vue](src/components/VoiceConsultationNew.vue) + [useEventListeners.ts](src/composables/useEventListeners.ts) + [safeHtml.ts](src/shared/lib/safeHtml.ts) + [http_server.rs](src-tauri/src/http_server.rs) + [med-hermes-sdk.js](sdk/med-hermes-sdk.js) + [api.md](./api.md)；独立入口来自 `/api/outpatient/emr/analyze` 或 `sdk.analyzeOutpatientEmr()`，HIS 每次必须传 `requestId + templateHtml + templateDefinition + targetFieldIds + recordContext`；语音一体化入口在 `startVoice` 中只额外携带严格 `outpatientEmr` 模板规格，由 workflow 在医生确认共享结果后生成病例上下文并打开同一预览。`outpatientEmrTemplateDefinition.ts` 只负责严格解码顶层章节 JSON，`outpatientEmrTemplate.ts` 按章节/字段稳定 ID 将定义与净化 HTML 合并并拒绝不一致，字典只来自结构定义，HTML 保留真实布局与当前显示值。`outpatientEmrRequestIdentity.ts` 将两份原文都纳入同 requestId 身份；`outpatientEmrTemplateSnapshotService.ts` 先按 `templateId + templateHash` 查询后台历史解析，命中时恢复字段快照，未命中时才登记两份原文和本地完整合并解析，后台只读展示；回参 builder 从同一医生最终值 Map 产生模板参数，合并模式保留结果页实际选择的标准回写 scope 并最终只发一条事件。旧 `outpatientEmrTemplateSource.ts`、`templateSource/sourceFormat`、重复解析失败兜底和单源解析均不保留；桌面端没有模板上传、粘贴或本地结果预览。 |
...
| 修改客户端更新源 / 测试构建 / 正式发版 / Win7 独立发布 | [发布流程](docs/release-process.md) + [UpdateChecker.vue](src/features/settings/ui/UpdateChecker.vue) + [ForceUpdateGate.vue](src/features/settings/ui/ForceUpdateGate.vue) + [updateCheckerStatus.ts](src/features/settings/model/updateCheckerStatus.ts) + [updateConfig.ts](src/services/updateConfig.ts) + [lib.rs](src-tauri/src/lib.rs) + [Win7 原生 region](src-tauri/src/win7_window_region.rs) + [Win7 region 协调器](src/app/shell/useWin7WindowRegion.ts) + [tauri.windows.conf.json](src-tauri/tauri.windows.conf.json) + [tauri.win7.conf.json](src-tauri/tauri.win7.conf.json) + [Cargo build-std 配置](src-tauri/.cargo/config.toml) + [Win7 Cargo runner](scripts/win7-cargo-runner.mjs) + [Windows runner 入口](scripts/win7-cargo-runner.cmd) + [Windows WiX 模板](src-tauri/windows/wix/main.wxs) + [test-release.mjs](scripts/test-release.mjs) + [test-build.yml](.github/workflows/test-build.yml) + [win7-test-build.yml](.github/workflows/win7-test-build.yml) + [win7-release-build.yml](.github/workflows/win7-release-build.yml) + [prepare-win7-release-config.mjs](scripts/prepare-win7-release-config.mjs) + [validate-win7-build.mjs](scripts/validate-win7-build.mjs) + [release.mjs](scripts/release.mjs) + [release.yml](.github/workflows/release.yml) + [release-preflight.mjs](scripts/release-preflight.mjs) + [validate-release-assets.mjs](scripts/validate-release-assets.mjs) + [package-windows-internal-update.mjs](scripts/package-windows-internal-update.mjs)；普通候选与 Win7 legacy 验证构建都只上传无发布元数据的 Actions Artifact。Win7 发布构建仍使用同一 `main` 源码和独立 flavor，但用独立密钥生成签名包，只能由后台上传到 `win7-testing / win7-production`；不得进入普通 `production / testing`、tag、GitHub Release 或 draft。Win7 flavor 固定 Tier 3 Rust target、上游 tauri-utils ctor 兼容修复、实机外置 WebView2 109、原生球态 region 裁剪和独立安装身份。测试通过后才允许在干净 `main` 上固化同一正式候选版本并创建正式 tag。强更策略是门禁事实源，门禁生效时即使 updater 没有返回安装包也不得展示“当前已是最新版本”；`main` 是唯一正式发布线，自 `1.4.0` 单调递增，draft 只有在统一更新清单与同批安装包 / 签名验证通过后才转为 latest；Windows release 产物内部名保持 `PCIE`，客户可见安装名由 WiX 模板统一为“全医慧助（PCIE）”，主升级码保持已发布 `PCIE` 安装线并兼容迁移历史 `MedHermes` / 早期中文安装线；内网发布端见 `../floating-ball-server/modules/release` |
...
| POST | `/api/outpatient/emr/analyze` | 门诊病历模板单次分析与字段参数回传 |
