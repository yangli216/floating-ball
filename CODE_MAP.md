...
| 修改门诊接诊与风险胶囊 | [useReceptionController.ts](src/app/events/useReceptionController.ts) + [receptionFlowGuard.ts](src/app/events/receptionFlowGuard.ts) + [receptionPatientSummary.ts](src/features/reception/lib/receptionPatientSummary.ts) + [useReceptionSessionController.ts](src/features/reception/model/useReceptionSessionController.ts) + [useOutpatientScenarioRouter.ts](src/features/reception/model/useOutpatientScenarioRouter.ts) + [ReceptionCapsule.vue](src/features/reception/ui/ReceptionCapsule.vue) + [features/reception-risk](src/features/reception-risk)；患者补全、风险评估和复诊机会异步写入必须复用同一 flow token，风险评估失败只降级风险能力，不得把已成功的患者接诊标记为失败；患者展示信息从 `currentPatient` 派生，风险和机会状态只经 session controller action 修改；患者字段兼容读取和病史摘要属于纯 lib，具体门诊场景导航由统一路由执行 |
| 修改复诊配药 | [chronicRefillAssessment.ts](src/features/reception-risk/lib/chronicRefillAssessment.ts) + [chronicRefillRecord.ts](src/features/reception-risk/api/chronicRefillRecord.ts) + [chronicRefillInventory.ts](src/features/reception-risk/lib/chronicRefillInventory.ts) + [useReceptionController.ts](src/app/events/useReceptionController.ts) + [HisAdapter.ts](src/services/his/HisAdapter.ts) + [PhisHisAdapter.ts](src/services/his/PhisHisAdapter.ts) + [hisService.ts](src/services/hisService.ts) + [clinicalResultContract.ts](src/features/clinical-result/clinicalResultContract.ts) + [VoiceConsultationNew.vue](src/components/VoiceConsultationNew.vue)；历史查询必须携带当前 `idVis` 以排除本次就诊；当前主诉/现病史/诊断出现报告回诊语义时必须抑制复诊配药候选，胶囊不得展示有限历史窗口内的具体就诊次数；草稿主诉/现病史必须表达具体慢病续方语义；药品按“库存同品 → 库存等效药 → 规范通用名兜底”处理，无库存标准名保持未选中；通过 `recommendationPolicy` 禁止结果页自动补拉药品、检查、检验或处置 |
| 修改门诊语音复诊聚合 | [outpatientFollowUpContext.ts](src/features/outpatient-follow-up/api/outpatientFollowUpContext.ts) + [OutpatientFollowUpPage.vue](src/features/outpatient-follow-up/ui/OutpatientFollowUpPage.vue) + [OutpatientFollowUpEvidencePanel.vue](src/features/outpatient-follow-up/ui/OutpatientFollowUpEvidencePanel.vue) + [HisAdapter.ts](src/services/his/HisAdapter.ts) + [types.ts](src/services/his/types.ts) + [PhisHisAdapter.ts](src/services/his/PhisHisAdapter.ts) + [hisService.ts](src/services/hisService.ts) + PHIS `AiInpatientEmrContextService.buildOutpatientFollowUpContext`；当前接诊上下文有效诊断是复诊分流前提；共享契约只允许 `followUpEligible / source / medicalRecordText / labReports / examReports / ineligibleReason`，检验报告保留完整项目，不返回患者、当前就诊、候选检查或 raw；独立复诊页左侧展示依据、右侧复用 `features/treatment-plan` 的推荐与回写能力 |
