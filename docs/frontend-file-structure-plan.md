...
| `VoiceConsultationNew.vue` 内一键回写回执 success / failed 分支 | `src/features/consultation-result/model/useWritebackFeedbackController.ts` | 结果页共享 controller：只管理已命中 requestId 回执的成功 / 失败分发和默认提示；页面仍负责调用 `complete_consultation`、缓存持久化、最终日志、成功后直接关闭结果页和 toast 注入，不再默认弹出问题反馈 |
...
| `VoiceConsultationNew.vue` 内推荐项反馈提交与结果页完成动作 | `src/features/voice-consultation/model/useVoiceFeedbackActions.ts` | 语音问诊专属 action controller：只编排推荐反馈 payload 提交、成功 toast、推荐弹层关闭和结果页完成回调；页面仍负责草稿状态、推荐目标登记、日志、缓存、PHIS 回写和成功回执时机 |
...
| `VoiceConsultationNew.vue` 内一键回写等待 / 回执状态 / banner 文案 | `src/features/consultation-result/model/useWritebackStatus.ts` | 结果页共享 composable：只管理当前 requestId、等待提示、最近回执和按钮 / banner 派生文案；页面仍负责 Tauri `listen`、`invoke('complete_consultation')`、成功/失败 toast、成功后结果页关闭和用户日志 |
| `VoiceConsultationNew.vue` 结果页样式块与双栏阅读导航 | `src/features/consultation-result/ui/ClinicalResultEditor.css` + `src/features/consultation-result/ui/ClinicalResultColumnNavigator.vue` + `src/features/consultation-result/model/useClinicalResultColumnNavigation.ts` | 样式已外置并承接桌面宽屏左右独立滚动、标题 / 导航吸顶和窄屏单栏回退；导航组件只渲染主诊断与分区入口，controller 只管理右栏滚动定位和当前分区，不得读取或修改诊断 / 治疗业务状态、PHIS 回写或窗口几何 |
...
35a. `features/voice-consultation/model/useVoiceFeedbackActions.ts`：只负责语音结果页推荐项反馈提交和结果页完成动作编排；提交函数、弹层关闭、toast、结果页关闭回调必须由页面显式注入，不能处理病例字段/整页反馈、读取缓存、提交用户日志、调用 PHIS 回写、触发 AI 或登记推荐目标。
