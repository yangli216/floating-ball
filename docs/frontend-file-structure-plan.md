...
| `VoiceConsultationNew.vue` / 独立鉴别诊断窗口内 checklist 解析与不匹配判断 | `src/features/clinical-result/diagnosisChecklist.ts` + `src/features/consultation-result/model/useClinicalResultDiagnosisChecklist.ts` | `clinical-result` 纯规则统一响应类型、条目归一、关键诊断不匹配判断和风险项映射；`consultation-result` controller 只管理共享结果页弹窗、请求生命周期、空态和错误态。页面注入 Prompt/LLM 请求、trace、错误文案与 toast；独立窗口保留自身折叠、watch 和窗口生命周期 |
