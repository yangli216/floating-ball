...
| `VoiceConsultationNew.vue` 内结果页渠道派生 | `src/features/consultation-result/model/useClinicalResultChannelStrategy.ts` | 结果页共享 strategy 第一刀：只根据 `voice/symptom` 派生日志类型、语音缓存开关、患者头展示和取消弹窗文案；页面仍负责缓存读写、日志提交、取消事件和 PHIS 回写 |
...
| `VoiceConsultationNew.vue` 内病例字段初始快照 / 修改判断 / 字段反馈展示状态 | `src/features/voice-consultation/model/useVoiceRecordFieldFeedbackState.ts` | 语音问诊专属 controller：只管理主诉 / 现病史 / 既往史 / 家族史的初始快照、当前值读取、人工修改判断、字段反馈 key、草稿读取和已提交标签；页面仍负责提交字段反馈、toast、用户日志、弹层开合和 PHIS 回写 |
...
| `ConsultationPage.vue` / `VoiceConsultationNew.vue` 内 `consultation-reference-feedback` 订阅与当前就诊过滤 | `src/features/consultation-result/model/useConsultationReferenceFeedbackListener.ts` | 结果页共享 listener：只管理事件名、`consultationId` 防串线和 Tauri 监听生命周期组合；页面仍负责 requestId 匹配、PHIS 回执状态写入、toast、缓存和日志 |
...
| `VoiceConsultationNew.vue` 内治疗推荐分组 / 空状态文案 | `src/features/consultation-result/model/useTreatmentSections.ts` | 结果页共享 composable：只根据治疗推荐列表、当前主诊断、刷新状态和上一版诊断 key 派生治疗分组、是否存在推荐和空状态文案；页面仍负责 AI 请求、刷新按钮、治疗项选择、库存校验、toast 和 PHIS 回写 |
...
| `src/components/VoiceRecommendationFeedbackPopover.vue` / `VoiceRecordFeedbackPopover.vue` / `VoiceSessionFeedbackBar.vue` | `src/features/voice-consultation/ui/*` | 已迁移并删除旧路径兼容包装，后续可随反馈 API 拆分再评估是否归入 `features/feedback` |
| `VoiceConsultationNew.vue` 内主诉 / 现病史 / 既往史字段编辑重复模板 | `src/features/voice-consultation/ui/VoiceRecordFieldEditor.vue` | 语音问诊专属受控 UI：只展示病例字段 textarea、人工修改标记、反馈按钮和 `VoiceRecordFeedbackPopover` 锚点；字段值、反馈开合、草稿更新、提交和 toast 仍由父页面注入 |
...
36. `features/consultation-result/model/useConsultationReferenceFeedbackListener.ts`：只负责订阅 `consultation-reference-feedback`、按当前 `consultationId` 过滤旧患者回执并把命中的 payload 交回页面；不能匹配 requestId、修改引用状态 map、弹 toast、提交日志或清理缓存。
...
34. `features/voice-consultation/model/useVoiceRecordFieldFeedbackState.ts`：只负责语音结果页病例字段初始快照、当前值读取、人工修改判断、字段反馈 key、草稿读取和已提交标签；字段反馈提交、toast、用户日志变更标记、弹层开合、缓存和 PHIS 回写仍由页面或既有反馈服务编排。
...
35a. `features/voice-consultation/model/useVoiceFeedbackActions.ts`：只负责语音结果页推荐项反馈、病例字段反馈和整页反馈的提交动作编排；提交函数、弹层关闭、toast、字段当前值读取、整页完成回调必须由页面显式注入，不能读取缓存、提交用户日志、调用 PHIS 回写、触发 AI 或登记推荐目标。
