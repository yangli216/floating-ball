...
24. **慢病长处方回写门禁**：只有 `chronic-refill` 结果页实际选择药品回写时，`record-confirmed` 才能携带 `prescriptionAttributes.chronicLongTerm = true`；无药回写和其他渠道必须省略。该中性属性不得改成 PHIS 私有 `slowMedicine` 并塞入药品明细。PHIS 必须复用签约状态与慢病诊断配置校验，校验通过后只给普通西药 / 中成药处方头设置 `slowMedicine = "1"`；校验失败明确回执失败，不得静默降级为普通处方。回归需同时核对数据库处方头、医生站勾选框和打印版式。
