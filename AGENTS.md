...
12. **Win7 legacy 发布线禁止混入普通正式发布**：Windows 7 只允许使用同一 `main` 源码上的 `tauri.win7.conf.json` 独立 flavor，不维护长期业务分支。`Win7 Legacy Test Build` 必须保持 Artifact-only、updater 关闭；只有 `Win7 Legacy Release Build` 可以用独立 Win7 updater 密钥生成签名发布包，并且只允许进入 `win7-testing / win7-production`。必须使用独立 identifier / UpgradeCode、外置安装的 WebView2 109、`x86_64-win7-windows-msvc + build-std`、独立 `latest.json / policy.json /` 回滚历史，禁止复用普通正式 `stable` target、UpgradeCode、签名密钥、`production / testing`、tag、Release 或 draft。GitHub 托管 Runner 禁止依赖内网发布服务可达性，触发人必须提供目标通道前置版本做离线 preflight，`WIN7_UPDATE_SERVER_BASE_URL` 只用于生成下载地址；内网 PCIE Server 必须按该通道历史最高版本执行最终单调递增门禁，降级只能走显式回滚入口。Win7 原生窗口 region 裁剪必须只由 `win7-legacy` Cargo feature 启用；任何主窗口几何变更前先恢复 full region，球态稳定后才能应用中心圆或五圆联合 region，不得让正式 Windows/macOS 路径启用该裁剪。Win7 包保留 `med-hermes` 深链时不得与普通正式客户端共机安装；发布包必须记录安全责任人和支持截止日期，且 Windows 7 SP1 x64 的 `N -> N+1` 实机升级冒烟完成前不得进入 `win7-production` 或描述为正式支持版本。
13. **门诊动态模板实验契约禁止隐式兼容**：`features/outpatient-emr` 在正式 HIS 接入前不承担早期实验协议的兼容义务。除非用户明确指定要兼容的旧版本和退出条件，否则契约收敛时必须直接删除旧字段名、别名解析、双读双写、自动范围推断及失败后的旧路径兜底；不得以 deprecated 分支、隐藏开关或“先保留不用”的死代码继续承载。
...
3. 最后附上验证结果：
   ...
   - Win7 legacy 验证附上固定 Rust 工具链 / target、外置 WebView2 109 版本、Artifact 名称与 Windows 7 SP1 x64 实机冒烟；Win7 发布额外附上独立通道、独立签名公钥指纹、源 commit、安全责任人、支持截止日期及 `N -> N+1` updater 实机结果，不得描述为普通 Windows 正式支持版本
