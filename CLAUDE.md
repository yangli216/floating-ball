...
- Endpoints:
  ...
  - `POST /api/consultation/stop`: Stops active consultation
   - `GET /api/consultation/events/ws`: Sole WebSocket stream for consultation event envelopes
...
5. **Submission:** Calls `complete_consultation` command (resultType=`record-confirmed`, payload built via [src/utils/recordConfirmedPayload.ts](src/utils/recordConfirmedPayload.ts)) → saves to `AppState` → HIS receives it via `/api/consultation/events/ws`
